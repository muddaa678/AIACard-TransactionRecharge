...
export type TransactionsTitleType = {
  className?: string;
};

const TransactionsTitle: NextPage<TransactionsTitleType> = ({
  className = "",
}) => {
  return (
    <div
      className={`self-stretch flex flex-row flex-wrap items-start justify-start gap-[3.375rem] max-w-full text-center text-[0.75rem] text-white font-montserrat mq700:gap-[1.688rem] mq950:flex-col ${className}`}
    >
      <div className="flex-1 flex flex-row items-start justify-start gap-[0.481rem] max-w-full mq700:min-w-full mq950:flex-wrap mq950:flex-[unset] mq950:self-stretch">
        <FrameComponent4 />
